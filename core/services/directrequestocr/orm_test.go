...
func TestORM_SetInTransmission(t *testing.T) {
	t.Parallel()

	orm := setupORM(t)
	id, _, _ := createRequest(t, orm)

	err := orm.SetInTransmission(id, []byte("result"), []byte("error"))
	require.NoError(t, err)
	...
	require.Equal(t, directrequestocr.IN_TRANSMISSION, req.State)
}
...
func TestORM_StateTransitions(t *testing.T) {
	...
	err = orm.SetInTransmission(id, nil, nil)
	require.Error(t, err)
	req, err = orm.FindById(id)
	require.NoError(t, err)
	require.Equal(t, directrequestocr.TIMED_OUT, req.State)
...
func TestORM_TimeoutExpiredResults(t *testing.T) {
	...
	// can time out IN_PROGRESS, RESULT_READY or IN_TRANSMISSION
	err := orm.SetResult(ids[0], 123, []byte("result"), now)
	require.NoError(t, err)
	err = orm.SetInTransmission(ids[1], []byte("result"), []byte(""))
	require.NoError(t, err)
