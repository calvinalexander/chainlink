...
// SetInTransmission provides a mock function with given fields: requestID, transmittedResult, transmittedError, qopts
func (_m *ORM) SetInTransmission(requestID directrequestocr.RequestID, transmittedResult []byte, transmittedError []byte, qopts ...pg.QOpt) error {
	_va := make([]interface{}, len(qopts))
	for _i := range qopts {
		_va[_i] = qopts[_i]
	}
