...
func newRequestInTransmission() drocr_serv.Request {
	return drocr_serv.Request{RequestID: newRequestID(), State: drocr_serv.IN_TRANSMISSION}
}
...
func TestDRReporting_ShouldAcceptFinalizedReport(t *testing.T) {
	...
	req3 := newRequestInTransmission()
	req4 := newRequestTimedOut()

	orm.On("FindById", req1.RequestID).Return(nil, errors.New("nonexistent ID"))
	orm.On("FindById", req2.RequestID).Return(&req2, nil)
	orm.On("SetInTransmission", req2.RequestID, mock.Anything, mock.Anything).Return(nil)
	orm.On("FindById", req3.RequestID).Return(&req3, nil)
	orm.On("SetInTransmission", req3.RequestID, mock.Anything, mock.Anything).Return(errors.New("same state"))
	orm.On("FindById", req4.RequestID).Return(&req4, nil)
	orm.On("SetInTransmission", req4.RequestID, mock.Anything, mock.Anything).Return(errors.New("already timed out"))
