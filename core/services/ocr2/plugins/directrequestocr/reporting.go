...
func (r *directRequestReporting) ShouldAcceptFinalizedReport(ctx context.Context, ts types.ReportTimestamp, report types.Report) (bool, error) {
	...
	for _, item := range decoded {
		...
		if err != nil {
			r.logger.Warn("directRequestReporting request doesn't exist locally", commontypes.LogFields{"requestID": reqIdStr})
			continue
		}
		err = r.pluginORM.SetInTransmission(sliceToByte32(item.RequestID), item.Result, item.Error) // validates state trasition
		if err != nil {
			r.logger.Debug("directRequestReporting state not changed to IN_TRANSMISSION", commontypes.LogFields{"requestID": reqIdStr})
			continue
		}
...
func (r *directRequestReporting) ShouldTransmitAcceptedReport(ctx context.Context, ts types.ReportTimestamp, report types.Report) (bool, error) {
	...
	for _, item := range decoded {
		...
		if request.State != directrequestocr.IN_TRANSMISSION {
			r.logger.Debug("directRequestReporting request is not IN_TRANSMISSION any more", commontypes.LogFields{"requestID": reqIdStr})
			continue
		}
		needTransmissionIds = append(needTransmissionIds, reqIdStr)
	}
